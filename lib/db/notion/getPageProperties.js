...
import { getNdEditorialCoverPreview } from '@/lib/site/ndEditorialCoverPreview'
...
export default async function getPageProperties(
  ...
  const previewCover = getNdEditorialCoverPreview(id)
  const pageCover = value?.format?.page_cover || previewCover
  properties.pageCover = mapImgUrl(pageCover, value) ?? ''
  properties.pageCoverThumbnail = mapImgUrl(pageCover, value, 'block') ?? ''
