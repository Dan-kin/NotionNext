...
function getCustomMenu({ collectionData, NOTION_CONFIG }) {
  ...
  // 允许站点在不修改 Notion 菜单记录的情况下，将旧的单篇入口升级为栏目入口。
  // 这让导航调整可通过 Git/Preview 审核并随时回滚。
  const routeOverrides = siteConfig(
  ...
  menus.forEach(menu => {
    ...
    if (routeOverrides?.[name]) {
      menu.href = routeOverrides[name]
      menu.target = '_self'
    }
  })

  const extraItems = siteConfig(
    'CUSTOM_MENU_EXTRA_ITEMS',
    [],
    NOTION_CONFIG
  )
  extraItems.forEach((item, index) => {
    if (!item?.name || menus.some(menu => (menu.name || menu.title) === item.name)) {
      return
    }
    const menu = {
    ...
    const beforeIndex = item.before
      ? menus.findIndex(existing => (existing.name || existing.title) === item.before)
      : -1
    if (beforeIndex >= 0) {
      menus.splice(beforeIndex, 0, menu)
    } else {
      menus.push(menu)
    }
  })
